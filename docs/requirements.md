...
| 8 | §10 | A routed leg that crosses water keeps two candidates, the ferry route and the ferry-avoiding land route, and chooses between them under `routing.ferry` (45 minutes of waiting per crossing; land unless the ferry is more than 15 minutes faster after that), with a per-leg `ferry_preference` override. Routers do not know sailing schedules, so the fastest route by sailing time sent short crossings by ferry where the bridge was the quicker trip in practice. The land route is cached under its own key, a leg with no ferry makes no second request, and a missing land route (HTTP 404) is remembered |
...
| 26 | §5, §10, §11 | Checked the real Anthropic usage console (9.15M input / 327K output tokens, 361 web searches, one day) and found two further real issues. First: `DEFAULT_PRICING_USD_PER_1M` had no entry for `claude-sonnet-5` — the no-match fallback in `_estimate_cost` is `return 0.0`, so every real Sonnet 5 call this session had been silently costed at exactly $0.00, which is why internal cost tracking looked cheap regardless of actual usage. Fixed with the confirmed real rate ($2/MTok input, $10/MTok output — verified live against `platform.claude.com/docs/en/about-claude/pricing`; web search is also billed separately at $10/1000 searches, not yet tracked). Second: `CLAUDE_SEARCH_TOOL`'s `max_uses: 5` permitted up to 5 agentic search rounds per call, each resending growing context as new input tokens (~25K tokens/search observed, consistent with multi-round compounding) — lowered to `max_uses: 1`. Also added a `--search-provider {grok,claude}` CLI flag that forces a single provider for both `url_discovery` and `cultural_events` and skips constructing a fallback client entirely, for a clean, uncontaminated per-provider cost/behavior comparison run. See `docs/design/search-provider-capability-probe.md` §10 |
...
| `var PWA_INSTALL_ENABLED = true;` | Set to `false` when `trip._meta.privacy_redacted` is true, suppressing the Install App button/prompt entirely (§13, §9 privacy redaction policy) |
...
Two different things have lived at the bottom of a generated itinerary, and this section
governs both. They are not the same block and the rules for them are opposite.

### 8.1 The attribution block — removed, and must not return

The original static attribution table (the "Made by Copilot" credit) is **not** appended to
generated itineraries. It was removed in v0.29 and nothing may reintroduce it.

Scenic-drive popups and other in-page modal content must not leak generator-version strings
or attribution-table boilerplate into their visible body copy.

### 8.2 The generator footer — required

A single generator footer **is** appended to every generated itinerary, replacing what §8.1
removed. It is the itinerary's provenance record and its only feedback channel, so it is
required rather than merely permitted, and it must carry all of the following:

| Element | Source | Notes |
|---|---|---|
| Generator name, linked to the repository | static | The link is the repository, not a personal or vendor credit |
...
```
Generated by Itinerary Generator v2.0.0 · Manifest: sw_manifest.yaml · Itinerary output: 2026-08-19 06:47 UTC
Issue reporting: Report broken links · Share itinerary feedback
```
...
qualification, which — once the Copilot table it referred to had been replaced by the
...
- The PWA Install App affordance (§13) must also be suppressed when privacy redaction is active: an installable home-screen app is a stronger distribution signal than is appropriate for a build that's intentionally omitting personal trip details.
...
| `routing.ferry.wait_minutes_per_crossing` | `45` | With `OPENROUTESERVICE_API_KEY` set, a leg whose route takes a ferry is also routed with ferries avoided and both routes are kept (`LegEstimate.ferry`, `.land`). This many minutes of waiting and boarding are added per crossing (one per `ferry_spans` entry) to the ferry route's routed time before the two are compared |
| `routing.ferry.prefer_land_within_minutes` | `15` | The land route is chosen unless the ferry route, allowance included, is more than this many minutes faster; exactly this many is still land. With no land route the ferry is chosen. A caller may override per leg with `ferry_preference`: `auto` (this rule), `avoid` (land whenever it exists), `prefer` (the ferry). The leg's miles, minutes and geometry are the chosen route's; `chosen` and `chosen_reason` record which and on what numbers |
...
- Install prompt UX should be exposed when browser eligibility allows, *except* when privacy redaction is active (§9), in which case the Install App button must never reveal itself and the browser's native install prompt must remain suppressed — offline caching/service worker registration are unaffected, only the install affordance.

...
| **OpenAI** | ✅ **The configured content fallback** (`ai.fallback_provider: openai`, `ai.fallback_model: gpt-4o-mini`, since 2026-09-21). $0.15/$0.60 per million — about 16x cheaper on output than the `claude-sonnet-5` it replaced, none of whose five fallback calls succeeded in that day's Old Hickory build — and it generated a whole guide's content on this machine the same day. Also selectable as `ai.provider`. | ❌ **Disqualified.** Probed with `gpt-5-search-api` (the current search-capable model — `gpt-4o-*-search-preview` deprecated 2026-07-23): 0/21 (0%) citation-matched — genuine search tool invoked, but returned URLs don't correspond to what was actually retrieved. Selectable in `search_provider.py` since 2026-08-15 (`94a5e56`, `openai_search.py`); no default uses it. | ❌ Same disqualification — selectable, and no default uses it. |

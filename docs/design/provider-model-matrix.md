...
|---|---|---|---|
| **Grok (xAI)** | ✅ Default (`ai.provider: grok`). Model: `grok-4-fast` (`config.yaml` `ai.model`, which beats env `XAI_MODEL`; `llm_client.py` `_provider_default_model` still answers `grok-latest` when nothing names one). Measured against `grok-latest` in §6. | ✅ **Primary role.** Model: `grok-latest` (env `XAI_MODEL`, `grok_search.py`). Evidence: 21/21 (100%) citation-matched URLs, real API validation. `url_discovery.search_provider: grok` (default). | ✅ **Cultural events** (`cultural_events.search_provider: grok`, since 2026-08-15; the module is off by default). Not the per-item fallback — Serper is (see below). |
...
**Two caveats on the cost figures.** They are the engine's own estimate, and
the run logs `no pricing entry for 'serper:serper'` -- search spend is outside
both numbers, and search has historically been the larger share of a run. The
comparison between the two columns is sound (both exclude the same thing); the
absolute figures are not a bill.

...
| v2.2-3a | `--privacy-mode` auto/on/off resolution (environment-based default, explicit override in either direction) | tests/test_main_requirements.py::test_resolve_privacy_redaction_auto_follows_environment; tests/test_main_requirements.py::test_resolve_privacy_redaction_explicit_override_wins | Tested | Keep. Note: an inverted on/off implementation bug was caught only by a manual CLI smoke test after unit tests already passed against the (wrong) implementation — see v2.2-2's recommendation; this is the same underlying gap |
| v2.2-3b | Redaction replaces `planning_links` with an explanatory placeholder and blanks `lodging.name`, leaving `lodging.location`/`checkin_time` untouched | tests/test_main_requirements.py::test_apply_privacy_redaction_replaces_planning_links_with_placeholder; tests/test_main_requirements.py::test_apply_privacy_redaction_blanks_lodging_name_keeps_location_and_checkin | Tested | Keep |
| v2.2-3c | Redacted planning-links pill renders as a non-link placeholder with an explanatory tooltip, not a real link and not silently absent | tests/test_html_assembler.py::test_header_links_renders_redacted_placeholder_not_a_link | Tested | Keep |
| v2.2-4 | PWA Install App affordance (button + native browser prompt) suppressed when privacy redaction is active | tests/test_html_assembler.py::test_assemble_disables_pwa_install_when_privacy_redacted | Tested | Keep. Note: initial implementation read the wrong `meta` variable (shadowed by `trip["trip"]` earlier in `assemble()`) and silently no-opped; caught by this same test on first run, not by inspection |

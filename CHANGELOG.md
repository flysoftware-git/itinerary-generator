...
- The frozen template gained the restaurant-anchor colour rule and nine
  restored `THEME_COLOR` placeholders, so a page rendered by it is a different
  artifact from one rendered before. `__template_version__` did **not** move
  at the time — nothing required it to, and it had read `2.5` since the
  initial rebuild while the template file changed repeatedly. That drift is
  what the template-version guard added in 2.5.0 exists to prevent; the
  version has since moved 2.5 → 2.5.7, each value recorded against its
  template checksum. The filename stays `v2.5_template.html`: it names the
  2.5 family, not a revision.

...
# The footer exactly as it stood before `trip.brand` existed. Written out in
# full, not built from the code under test, because the whole claim of the
# change is that a manifest without a brand block renders what it always did --
# and a test that derives its expectation from the implementation cannot check
# that. If this literal ever needs editing, the change was not additive.
_FOOTER_BEFORE_BRAND = (
    ...
    ' v9.9.9 \u00b7 Itinerary output: 2026-07-26 17:41 UTC'
    '<div style="margin-top:0.35rem;">'
    'Issue reporting: '
    '<a href="https://github.com/flysoftware-git/itinerary-generator/issues/new'
...
def _trip_with_brand(brand=None):
    trip = {
        "trip": {"title": "Test Trip", "theme_color": "#C0623E"},
        "_meta": {
            "generator_version": "9.9.9",
    ...
    if brand is not None:
        trip["trip"]["brand"] = brand
    return trip


def test_footer_is_unchanged_without_a_brand_block() -> None:
    """The ordinary case, byte for byte."""
    assembler = HTMLAssembler(config_path="config.yaml")
...
def test_brand_distributor_is_credited_above_provenance() -> None:
    assembler = HTMLAssembler(config_path="config.yaml")

    footer = assembler._build_generator_footer(
        _trip_with_brand({"distributor": "Acme Travel",
                          "distributor_url": "https://acme.example"})
    )

    assert '<a href="https://acme.example">Acme Travel</a>' in footer
    assert footer.index("Prepared by") < footer.index("Generated by")


def test_brand_support_url_replaces_both_issue_links() -> None:
    assembler = HTMLAssembler(config_path="config.yaml")

    footer = assembler._build_generator_footer(
        _trip_with_brand({"support_url": "mailto:help@acme.example",
                          "support_label": "Email us"})
    )

    assert 'Support: <a href="mailto:help@acme.example">Email us</a>' in footer
    assert "Report broken links" not in footer
    assert "Share itinerary feedback" not in footer


def test_provenance_survives_a_full_brand_block() -> None:
    """SS8.2 is not configurable. Whatever a distributor puts on the page, the
    page still says what built it, from which manifest, and when."""
    assembler = HTMLAssembler(config_path="config.yaml")
    ...
    assert ">Itinerary Generator</a>" in footer
    assert "https://github.com/flysoftware-git/itinerary-generator" in footer
    assert "v9.9.9" in footer
    assert "Itinerary output: 2026-07-26 17:41 UTC" in footer


def test_brand_values_are_escaped() -> None:
    assembler = HTMLAssembler(config_path="config.yaml")

    footer = assembler._build_generator_footer(
        _trip_with_brand({"distributor": '<script>alert("x")</script>'})
    )
...
def test_brand_urls_with_unexpected_schemes_are_dropped() -> None:
    """The schema rejects these at parse time; a hand-built trip dict does not
    go through the schema, and a footer is not worth failing an eighteen-minute
    run over. Dropping the value renders the page it would have been anyway."""
    assembler = HTMLAssembler(config_path="config.yaml")

    footer = assembler._build_generator_footer(
        _trip_with_brand({"distributor": "Acme Travel",
                          "distributor_url": "javascript:alert(1)",
                          "support_url": "javascript:alert(2)"})
    )

...
class HTMLAssembler:
    ...
    def _build_generator_footer(
        ...
        return (
            ...
            f' v{html_escape.escape(str(version))}{self._build_commit_segment(meta)}{manifest_segment}'
    ...
    @staticmethod
    def _build_commit_segment(meta: dict[str, Any]) -> str:
        """The commit that built the page, beside the version, for the footer.
        ...
        between them, and a reader holding one cannot tell which. The commit was
        ...
        not something a reader sees, so a question about a guide in hand could
        not be traced to the code that made it.

        Shown as the short commit, with *local changes* when the tree was dirty:
        a dirty build's commit is not the whole story, and saying so is what
        keeps the stamp honest. Empty when there is no git -- a copy installed
        from an archive -- rather than a placeholder that looks like a commit.
        """
        ...
        if not short:
            return ""
        dirty = " + local changes" if git.get("dirty") else ""
        return f" · {html_escape.escape(short)}{dirty}"
